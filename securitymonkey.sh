...
if [ "${email}" == "" ]; then
  echo "Email is not passed. Please set it as dockerrun -e mail=info@xervmon.com"
  email="support@xervmon.com"
fi
...
sed -i "s/securityteam@xervmon.com/$email/g" /home/ubuntu/security_monkey/env-config/config-deploy.py
sed -i "s/securitymonkey@xervmon.com/$email/g" /home/ubuntu/security_monkey/env-config/config-deploy.py
